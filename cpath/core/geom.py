...
__all__ = ['polygon_area',
           'make_tform', 'make_affine', 'make_bilinear', 'make_polynomial',
           'make_projective', 'make_similarity', 'TRANSFORMATIONS',
           'affine_transform', 'bilinear_transform', 'polynomial_transform',
           'projective_transform', 'similarity_transform', 'Transformation']
...
def random_patch(patch_shape: tuple, contour: np.array):
    """
    Yields an infinite series of rectangles within a polygonal area.

    :param patch_shape: (width, height) of the rectangles to be returned
    :param contour: (numpy.array (Nx2)) an array of vertices (first and last
        row do not need to be identical) specifying the polygonal line bounding
        the region for sampling
    """
    poly = Polygon(contour)
    xmin, ymin, xmax, ymax = poly.bounds
    if xmax - xmin < patch_shape[0] or ymax - ymin < patch_shape[1]:
        raise RuntimeWarning("Ara too small for the patch size")

    while True:
        x0 = rnd.random_integers(low=xmin+1, high=xmax-patch_shape[0]-1)
        y0 = rnd.random_integers(low=ymin+1, high=ymax-patch_shape[1]-1)
        if poly.contains(box(x0, y0, x0+patch_shape[0], y0+patch_shape[1])):
            yield (x0, y0, x0+patch_shape[0], y0+patch_shape[1])


def polygon_area(x: np.array, y: np.array) -> float:
    """
    Computes surface of a polygon given in terms of its vertices (x, y)

    :param x: (numpy.array) x-coordinates of the vertices
    :param y: (numpy.array) y-coordinates of the vertices
...
# From: https://raw.githubusercontent.com/ahojnnes/numpy-snippets/master/transformations.py
